...
input (`Await`) or signaling termination via `Halt`.
  ...
  sealed trait Process[I, O] {
    ...
    def |>[O2](p2: Process[O, O2]): Process[I, O2] = this match {
      case Emit(head, tail) => p2 match {
        ...
        case Await(recv) => tail |> recv(Some(head))
        case Halt() => Halt[I, O2]
      }
      case Await(recv) => Await(option => recv(option) |> p2)
      case Halt() => p2 match {
        case Emit(h, t) => Emit(h, this |> t)
        case _ => Halt[I, O2]
      }
  ...
  object Process {
    ...
    def take[I](n: Int): Process[I, I] = {
      def helper(alreadyTaken: Int): Process[I, I] = alreadyTaken match {
        case _ if alreadyTaken < n => Await { option =>
          option.map(i => Emit(i, helper(alreadyTaken + 1))) getOrElse Halt[I, I]
        }
        case _ => Halt[I, I]
      }
      helper(0)
    ...
    def drop[I](n: Int): Process[I, I] = {
      def helper(alreadyDropped: Int): Process[I, I] = alreadyDropped match {
        case _ if alreadyDropped < n => Await { option =>
          option.map(_ => helper(alreadyDropped + 1)) getOrElse Halt[I, I]
        }
        case _ => Await { option =>
          option.map(i => Emit(i, helper(alreadyDropped))) getOrElse Halt[I, I]
        }
      }
      helper(0)
    }

    def takeWhile[I](f: I => Boolean): Process[I, I] = Await { option =>
      option.map(i => if (f(i)) Emit(i, takeWhile(f)) else Halt[I, I]) getOrElse Halt[I, I]
    }

    def dropWhile[I](f: I => Boolean): Process[I, I] = Await { option =>
      option.map(i => if (f(i)) dropWhile(f) else Emit(i, dropWhile(f))) getOrElse Halt[I, I]
    }
    ...
    def count[I]: Process[I, Int] = {
      def helper(c: Int): Process[I, Int] = Await { option =>
        option.map(Function.const(Emit(c, helper(c + 1)))) getOrElse Halt[I, Int]
      }
      helper(1)
    }
...
object SimpleStreamTransducersMain extends App {

  import SimpleStreamTransducers.Process._

  println {
    take(5)((1 to 10).toStream).toList
  }
  println {
    drop(2)((1 to 10).toStream).toList
  }
  println {
    takeWhile[Int](i => i < 8)((1 to 10).toStream).toList
  }
  println {
    dropWhile[Int](i => i < 8)((1 to 10).toStream).toList
  }
  println {
    count((1 to 10).map(Function.const("jo")).toStream).toList
  }
  println {
    (take(100) |> drop[Int](7))((1 to 10).toStream).toList
  }
}
...
object ProcessTest extends App {
  ...
  println {
    //    Process.collect(Process.convertAll)
  }
}
