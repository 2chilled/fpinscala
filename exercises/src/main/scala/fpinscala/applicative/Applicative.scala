...
trait Applicative[F[_]] extends Functor[F] {
  ...
  def traverse[A, B](as: List[A])(f: A => F[B]): F[List[B]] = as.foldRight(unit(Nil: List[B])) {
    case (a, fl) =>
      val g = (l: List[B], b: B) => b :: l
      apply(map(fl)(g.curried))(f(a)) //map2(f(a), fl)(_ :: _)
  }

  def replicateM[A](n: Int, fa: F[A]): F[List[A]] = {
    @tailrec
    def helper(n: Int, target: F[List[A]] = unit(Nil)): F[List[A]] = {
      if (n > 0) helper(n - 1, apply(map(target)(l => (a: A) => a :: l))(fa))
      else target
    }
    helper(n)
  }
  ...
  def sequenceMap[K, V](ofa: Map[K, F[V]]): F[Map[K, V]] = ofa.foldLeft(unit(Map.empty[K, V])) {
    case (fmkv, (k, fv)) => map2(fmkv, fv)((mkv, v) => mkv + (k -> v))
  }
}
